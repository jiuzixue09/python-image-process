...
Stats = namedtuple('Stats', 'id log_id response_success_count response_error_count request_redirect_count '
...
def insert(rows):
    ...
    sql = """insert into stats(log_id,response_success_count,response_error_count,request_redirect_count, 
    ...
    for data in rows:
        ...
        try:
            db.insert_data(sql, log_id, data['responseSuccessCount'], data['responseErrorCount'],
                           ...
                           data['parseErrorCount'], data['parseItemCount'], datetime.now(), data['type'])
...
def fetch() -> List[Stats]:
    ...
    rows = db.find_where('select * from stats where log_id > ?', log_id)
    db.close_db()
